...
save_path = path.join(save_folder, "model28.ckpt")
...
def generate_predictions(b=150, s=100000):
...
def load():
    _path = path.join(folder, "predictions28.npy")
    a = np.load(_path)
    return a, None
...
def draw(at, pwid=45, thr=0.02, conv=25):
    plt.cla()
    N = 500
    x = np.arange(N)
    z = all_predictions[at:at+N]
    z_conv = np.convolve(z, np.full(conv, 1/conv), 'same')
    zf = find_peaks(z_conv, pwid, thr)
    plt.cla()
    plt.plot(z)
    plt.plot(z_conv)
    zf = np.round(zf).astype(int) - 3
...
##
##def observe(pwid=50, thr=0.1, conv=30):
##    for x in range(10000):
##        draw(x*10, pwid=50, conv=30, thr=0.1)
##        plt.pause(0.00001)
...
#all_predictions, audio_filename = generate_predictions(b=150)
...
conv = 25
thr = 0.02
peaks = find_peaks(np.convolve(all_predictions, np.full(conv, 1 / conv), 'same'), pwid, thr)
bpm, offset = find_timing(peaks)
snapped_peaks = snap_peaks(peaks, bpm, offset)
notes = generate_beats(snapped_peaks)
export_map(notes, bpm, offset, audio_filename)
